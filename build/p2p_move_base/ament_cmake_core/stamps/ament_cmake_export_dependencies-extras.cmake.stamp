...
set(_exported_dependencies "rclcpp;rclcpp_action;rclcpp_lifecycle;rclcpp_components;std_msgs;std_srvs;sensor_msgs;visualization_msgs;nav_msgs;geometry_msgs;builtin_interfaces;message_filters;tf2_ros;tf2;tf2_geometry_msgs;tf2_eigen;pluginlib;pcl_conversions;angles;local_planner;recovery_behaviors;dddmr_sys_core")

find_package(ament_cmake_libraries QUIET REQUIRED)

# find_package() all dependencies
# and append their DEFINITIONS INCLUDE_DIRS, LIBRARIES, and LINK_FLAGS
# variables to p2p_move_base_DEFINITIONS, p2p_move_base_INCLUDE_DIRS,
# p2p_move_base_LIBRARIES, and p2p_move_base_LINK_FLAGS.
# Additionally collect the direct dependency names in
# p2p_move_base_DEPENDENCIES as well as the recursive dependency names
# in p2p_move_base_RECURSIVE_DEPENDENCIES.
if(NOT _exported_dependencies STREQUAL "")
  find_package(ament_cmake_core QUIET REQUIRED)
  set(p2p_move_base_DEPENDENCIES ${_exported_dependencies})
  set(p2p_move_base_RECURSIVE_DEPENDENCIES ${_exported_dependencies})
  set(_libraries)
  foreach(_dep ${_exported_dependencies})
    ...
    if(NOT "${${_dep}_TARGETS}" STREQUAL "")
      foreach(_target ${${_dep}_TARGETS})
        ...
        if(TARGET "${_target}")
          ...
          if(_include_dirs)
            list_append_unique(p2p_move_base_INCLUDE_DIRS "${_include_dirs}")
          endif()
    ...
    if(NOT use_modern_cmake)
      if(${_dep}_DEFINITIONS)
        list_append_unique(p2p_move_base_DEFINITIONS "${${_dep}_DEFINITIONS}")
      endif()
      if(${_dep}_INCLUDE_DIRS)
        list_append_unique(p2p_move_base_INCLUDE_DIRS "${${_dep}_INCLUDE_DIRS}")
      endif()
      if(${_dep}_LIBRARIES)
        list(APPEND _libraries "${${_dep}_LIBRARIES}")
      endif()
      if(${_dep}_LINK_FLAGS)
        list_append_unique(p2p_move_base_LINK_FLAGS "${${_dep}_LINK_FLAGS}")
      endif()
      if(${_dep}_RECURSIVE_DEPENDENCIES)
        list_append_unique(p2p_move_base_RECURSIVE_DEPENDENCIES "${${_dep}_RECURSIVE_DEPENDENCIES}")
      endif()
    endif()
    if(_libraries)
      ament_libraries_deduplicate(_libraries "${_libraries}")
      list(APPEND p2p_move_base_LIBRARIES "${_libraries}")
    endif()
  endforeach()
endif()
